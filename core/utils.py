...
def debug_image(nets, args, inputs, step):
    ...
    filename = ospj(args.sample_dir + args.folder_name, '%06d_cycle_consistency.jpg' % (step))
    ...
    #     filename = ospj(args.sample_dir + args.folder_name, '%06d_latent_psi_%.1f.jpg' % (step, psi))
    #     translate_using_latent(nets, args, x_src, y_trg_list, z_trg_list, psi, filename)

    # reference-guided image synthesis
    filename = ospj(args.sample_dir + args.folder_name, '%06d_reference.jpg' % (step))
    translate_using_reference(nets, args, x_src, x_ref, y_ref, filename)
    
    filename = ospj(args.sample_dir + args.folder_name, '%06d_reference_opposite.jpg' % (step))
    translate_using_reference(nets, args, y_src, y_ref, x_ref, filename)
