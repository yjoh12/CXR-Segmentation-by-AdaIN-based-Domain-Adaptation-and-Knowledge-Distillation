...
from core.segmentation import post_processing
...
def main(args):
    ...
    val_loader = get_eval_loader(root=os.path.join(args.val_img_dir, args.domain0), img_size=args.img_size, batch_size=args.val_batch_size, shuffle=False, num_workers=args.num_workers,args=args)
    ...
    if args.flag_self:
        _load_checkpoint_pretrain(step=1000000, dir=args.checkpoint_dir.replace('+self/','/'), **nets)   
    ...
    # eval
    if args.mode == 'eval':
        _load_checkpoint_pretrain(step=args.resume_iter, dir=args.checkpoint_dir, **nets)  
        y_src_unpaired = torch.tensor([DOMAIN_UNPAIRED]).to(device)
        y_self = torch.tensor([DOMAIN_UNPAIRED]).to(device)
        ...
        for i, input in enumerate(val_loader):
            ...
            if args.flag_self:
                z_self = torch.tensor([[0.25,0.5,0.75,1.0]]).to(device)
                s_trg_src = nets.mapping_network(z_self, y_self) 
                out = nets.generator(input, y_src_unpaired, y_trg_msk, s=[], seg=True, self_cons=s_trg_src)
            else:
                s_src = nets.style_encoder(x_src_ref, y_trg_src)
                out = nets.generator(input, y_src_unpaired, y_trg_src, s_src)
                out = nets.generator(out, y_trg_src, y_trg_msk, s=[], seg=True) 
            out = torch.argmax(out, dim=1).detach().cpu().squeeze().numpy().astype('uint8')
        
            # post processing
            out = out*255
            out_post = Image.fromarray(post_processing(out))
            out_post.save(args.result_dir + val_loader.dataset.samples[i].stem + "_mask.png")
...
def _load_checkpoint_pretrain(step, dir='', **nets):
...
if __name__ == '__main__':
    ...
    # self consistency
    parser.add_argument('--flag_self', type=bool, default=True)
    ...
    parser.add_argument('--resume_iter', type=int, default=1000000, help='Iterations to resume training/testing')
    ...
    parser.add_argument('--val_img_dir', type=str, default=root_dir)
    parser.add_argument('--train_img_dir', type=str, default=root_dir) 
    args = parser.parse_args()
    
    # self
    if args.flag_self:
        save_dir = save_dir.replace('/', '+self/')
        if args.mode == 'eval':
            args.resume_iter = 2000000
        args.total_iters = args.resume_iter + args.total_iters
        args.eval_every = int(args.eval_every/5) 
        
    # directories
    args.checkpoint_dir ='expr/'+save_dir
    args.result_dir = 'expr/'+save_dir+'result/'
